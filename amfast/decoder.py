...
class Decoder(object):
    ...
    def _getContext(self, input):
        return DecoderContext(input, amf3=self.amf3, class_def_mapper=self.class_def_mapper);

    def decode(self, val):
        """Decode a string or file-like-object from AMF."""
        return decode(self._getContext(val))

    def decode_packet(self, val):
        """Decode a string or file-like-object representing an AMF packet."""
        return decode_packet(self._getContext(val))
